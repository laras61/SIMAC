...
<body>
    ...
    @php
        $jenisOptions = ['maintenance', 'perbaikan'];
        $statusOptions = ['pending', 'terkirim', 'gagal'];
    @endphp
    <div class="wrap">
        <div class="stack">
            <div class="page-head">
                @if ($editItem)
                    <form method="POST" action="{{ route('remainder.update', $editItem->id_remainder) }}">
                        @csrf
                        @method('PATCH')
                        <div class="form-grid">
                            <div class="field">
                            ...
                            <div class="field">
                                ...
                                <input id="tanggal_kirim" type="date" name="tanggal_kirim" value="{{ old('tanggal_kirim', $editItem->tanggal_kirim) }}" required>
                            </div>
                            <div class="field">
                                <label for="jenis">Jenis</label>
                                @php $jenisEdit = old('jenis', $editItem->jenis); @endphp
                                <select id="jenis" name="jenis" required>
                                    @foreach ($jenisOptions as $jenis)
                                        <option value="{{ $jenis }}" {{ $jenisEdit === $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="field">
                                <label for="email_tujuan">Email Tujuan</label>
                                <input id="email_tujuan" type="email" name="email_tujuan" value="{{ old('email_tujuan', $editItem->email_tujuan) }}" required>
                            </div>
                            <div class="field">
                                <label for="status_kirim">Status Kirim</label>
                                @php $statusEdit = old('status_kirim', $editItem->status_kirim); @endphp
                                <select id="status_kirim" name="status_kirim" required>
                        ...
                        <div class="actions">
                            ...
                            <a href="{{ route('remainder.index') }}" class="btn btn-soft">Batal Edit</a>
                        </div>
                    </form>
                @endif
            </div>
            ...
            <div class="panel">
                <div class="panel-head">
                    ...
                    <button type="button" class="btn btn-primary" onclick="openTambahModal()">Tambah Reminder</button>
                </div>
                <p class="sub">Total data: {{ $items->count() }}</p>
                <form method="GET" action="{{ route('remainder.index') }}" class="filter-form" id="remainderFilterForm">
                    <div class="field">
                        <label for="q">Pencarian</label>
                        <input id="q" type="text" name="q" value="{{ request('q') }}" placeholder="Aset, email, jenis, status">
                    </div>
                    <div class="field">
                    ...
                    <tbody>
                        @forelse ($items as $row)
                            <tr>
                                ...
                                <td>{{ $row->jenis }}</td>
                                <td>{{ $row->email_tujuan }}</td>
                                <td>{{ $row->status_kirim }}</td>
                                <td>
                                    <div class="row-actions">
                                        <a href="{{ route('remainder.index', array_merge(request()->only(['q', 'status']), ['edit' => $row->id_remainder])) }}" class="btn btn-warning">Edit</a>
                                        <form method="POST" action="{{ route('remainder.destroy', $row->id_remainder) }}" class="delete-form" data-item="{{ optional($row->barang)->kode_bmn ?? ('ID ' . $row->id_remainder) }}">
                                            @csrf
                                            @method('DELETE')
    ...
    <div id="tambahRemainderModal" class="modal {{ ($errors->any() && !$editItem) ? 'open' : '' }}">
        <div class="modal-card">
            <div class="modal-head">
                <h2>Tambah Reminder</h2>
                <button type="button" class="btn btn-close" onclick="closeTambahModal()">Tutup</button>
            </div>
            <form method="POST" action="{{ route('remainder.insert') }}">
                @csrf
                <div class="form-grid">
                    <div class="field">
                    ...
                    <div class="field">
                        <label for="add_tanggal_kirim">Tanggal Kirim</label>
                        <input id="add_tanggal_kirim" type="date" name="tanggal_kirim" value="{{ old('tanggal_kirim') }}" required>
                    </div>
                    <div class="field">
                        <label for="add_jenis">Jenis</label>
                        @php $jenisTambah = old('jenis', 'maintenance'); @endphp
                    ...
                    <div class="field">
                        <label for="add_email_tujuan">Email Tujuan</label>
                        <input id="add_email_tujuan" type="email" name="email_tujuan" value="{{ old('email_tujuan') }}" required>
                    </div>
                    <div class="field">
                        <label for="add_status_kirim">Status Kirim</label>
                        @php $statusTambah = old('status_kirim', 'pending'); @endphp
                        <select id="add_status_kirim" name="status_kirim" required>
                            @foreach ($statusOptions as $statusOption)
    ...
    <script>
        ...
        function openTambahModal() {
            document.getElementById('tambahRemainderModal').classList.add('open');
        }

        function closeTambahModal() {
            document.getElementById('tambahRemainderModal').classList.remove('open');
        }

        (function initAutoFilter() {
            const form = document.getElementById('remainderFilterForm');
            if (!form) return;
